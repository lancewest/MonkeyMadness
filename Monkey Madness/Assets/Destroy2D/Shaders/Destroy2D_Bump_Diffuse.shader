Shader "Destroy2D/Bumped Diffuse" {
	Properties {
	...
	SubShader
	{
		...
		{ 
			...
			float2 normalTex = TRANSFORM_TEX(i.uv2_ColorMask, _NormalMap);
			float4 normalColor = tex2D(_NormalMap,  float2(normalTex.x, 1-normalTex.y));
